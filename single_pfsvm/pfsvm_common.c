...
int harris_direction(IMAGE *dec, int i, int j)
{
    int sobel_x[9] = {
        -1, 0, 1,
        -2, 0, 2,
    ...
    for (s = -1; s <= 1; s++)
    {
        for (t = -1; t <= 1; t++)
        {
            ...
            if (x < 0)
                x = 0;
            else if (x > dec->width - 1)
                x = dec->width - 1;
            y = i + s;
            if (y < 0)
                y = 0;
            else if (y > dec->height - 1)
                y = dec->height - 1;

            tmp += dec->val[y][x] * sobel_x[z];
            z++;
        }
    }
    x_direction = tmp;
    ...
    if(abs(x_direction) > abs(y_direction)){
        if(x_direction < 0)direction = 3;
        else direction = 2;
    }
    else if(abs(x_direction) < abs(y_direction)){
        if(y_direction < 0)direction = 0;
        else direction = 1;
    }
...
/*特徴ベクトルの取得*/
int get_fvector_harris(IMAGE *img, int i, int j, double gain, double *fvector, int direction)
{
    typedef struct
    {
        int y, x;
